...
call :log [3/4] Starting PhoBiHSD GPU container ...
...
call :log PhoBiHSD GPU is starting at http://localhost:%PORT%
call :log Note: first run may need a few minutes to download pretrained model.
exit /b 0

:error
call :log Failed to run GPU container. Ensure Docker Desktop + NVIDIA runtime are ready.
exit /b 1
